define(['jquery', 'jqueryCookie', 'template', 'nprogress'], function ($, undefined, template, nprogress) {
    // 左上角的用户信息展示
    (function () {
        /**
         * 1、获取本地存储的用户信息
    ...
    (function () {
        ...
        /**
         * 还有一些子页面或隐藏较深的页面,这些页面在导航左侧没有对应的列表,那么我们可以手动添加一些配置,单独指定那些页面应该对应哪个a标签
         * 1.把所有页面的路径与对应的左侧列表href使用key,value的方式映射
         * 2.获取当前页面的路径
         * 3.用该路径去对应的配置中查找
         *   3.1 如果找到对应的配置值,则使用这个路径去找对应的a标签
         *   3.2 如果没有找到,则直接使用这个路径去找对应的a标签
         * 4.移除所有a标签的active类名
